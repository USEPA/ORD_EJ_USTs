...
# Remove tanks older than 1970 but keep NA values
...
# remove releases older than 1970 but keep NA values, also remove uncertain locations.
...
mutate(GISJOIN = paste0("G",substr(ID,1,2),"0",substr(ID,3,5),"0",substr(ID,6,12)))%>% # Create GISJOIN ID in case we want to use NHGIS data later
  select(GISJOIN,ID,STATE_NAME,ACSTOTPOP,MINORPCT,LOWINCPCT,LESSHSPCT,LINGISOPCT,UNDER5PCT,OVER64PCT,VULEOPCT,AREALAND)%>% # Trim to needed columns
...
nEJ <- vroom("D:/data/EJ/EJSCREEN_2020_USPR.csv")%>%
  mutate(GISJOIN = paste0("G",substr(ID,1,2),"0",substr(ID,3,5),"0",substr(ID,6,12)),
         N_P_MINORPCT_BIN = ifelse(as.numeric(P_MINORPCT) <= 10,"0-10",
...
sEJ <- vroom("D:/data/EJ/EJSCREEN_2020_StatePctile.csv")%>%
  mutate(GISJOIN = paste0("G",substr(ID,1,2),"0",substr(ID,3,5),"0",substr(ID,6,12)),
...
write.csv(bins,here("projects/EJ/Article/data/BGs_All_Attributes_fix.csv"))
bins <- read.csv(here("projects/EJ/Article/data/BGs_All_Attributes_fix.csv"))
...
for(n in 19:32){
  ...
  for (i in 33:36) {
    ...
    # 
     ...
     orca(p, file = paste0("projects/EJ/Article/figures/national/Bin_Box_Plots","/",yVar,"_by_",xVar,".png"))
