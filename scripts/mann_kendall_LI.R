...
for (n in 16) {
  ...
  for (i in 17:20) {
    ...
    write.csv(usMK2, "/work/TANKSDB/github/UST_groundwater_vulnerability_model/projects/EJ/Article/data/US_All_Mann_Kendall_LI.csv")
    
    print(paste0("Completed: ",colnames(allbgs)[n]," with ", colnames(allbgs)[i]," --- ",Sys.time()))
  }
}

print(paste0("Completed all Low-Income Mann-Kendall Calculations at ", Sys.time()))
