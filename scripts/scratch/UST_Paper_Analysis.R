...
tanks <- st_read("D:/data/USTs/Public.gdb", layer = "USTs")%>%
  filter(Installation_Date >= lubridate::mdy("01-01-1970") | is.na(Installation_Date) | as.character(Installation_Date) == "<NA>") # remove tanks older than 1970 but keep NA values
...
releases <- st_read("D:/data/USTs/Public.gdb", layer = "Releases")%>%
  ...
  filter(Reported_Date >= lubridate::mdy("01-01-1970") | is.na(Reported_Date) | as.character(Reported_Date) == "<NA>")%>% # remove releases older than 1970 but keep NA values
  st_transform(5070)
  
  # NEED TO RECREATE DATA FOR BOYCE WITH COMPLTE FACILITIES TANKS RELEASES, MAKE SURE NA VALUES ARE PRESENT
  # MAKE SURE MISSISSIPPI AND OTHERS HAVE CORRECT NUMBERS
...
ggplot(bgTally)+
  geom_point(aes(x = Pct_Minority, y = N_Facilities / (ALAND/100000)), alpha = .02)

lm <- lm(bgTally$N_Releases~bgTally$Pct_Minority)



# Check count of minority block groups by state
minBg <- bgTally%>%
  filter(Pct_Minority >= 90)
table(minBg$STATE)
...
## SCRATCH STATS ##
###################
# Filter >90% minority & at least one facility
filt1 <- bgTally%>%
  filter(Pct_Minority >= 90 & N_Facilities > 0)
filt2 <- bgTally%>%
  filter(Pct_Minority >= 90)
nrow(filt1) / nrow(filt2)

# Filter <= 10% & at least one facility
filt3 <- bgTally%>%
  filter(Pct_Minority <= 10 & N_Facilities > 0)
filt4 <- bgTally%>%
  filter(Pct_Minority <=10)
nrow(filt3)/nrow(filt4)

# Top 1% of minority block groups have same number of releases as bottom X%
tenPercntMinority <- quantile(bgTally$Pct_Minority, probs = (.9), na.rm = TRUE)

minRel <- bgTally%>%
  filter(Pct_Minority >= 70.34743)
sum(minRel$N_Releases, na.rm = TRUE)

# 207416 releases in the top 10% of minority block groups
n <- 1
i <- 1
while(n < 207416){
  filt <- bgTally%>%
    filter(Pct_Minority < i)
  n <- sum(filt$N_Releases)
  print(paste0(i," minority has ",n," releases"))
  i <- i+.5
}


# Let's take a look at histograms of breakdowns by state
ggplot(df)+
  geom_histogram(aes(x = Pct_Minority), bins = 10)+
  facet_wrap(facets = ~STATE, scales = "free")+
  labs(title = "Percent Minority Block Groups by State", x = "Percent Minority", y = "Count of Block Groups")

# Let's take a look at histograms of breakdowns by state
ggplot(df)+
  geom_histogram(aes(x = Pct_Low_Income), bins = 10)+
  facet_wrap(facets = ~STATE, scales = "free")+
  labs(title = "Percent Low-Income Block Groups by State", x = "Percent Low-Income", y = "Count of Block Groups")
