...
for (n in 32:34) {
  ei <- allbgs%>%
    select(GIS_Tract,colnames(allbgs)[n],ACSTOTPOP,MINORPCT, LOWINCPCT, LESSHSPCT, LINGISOPCT, UNDER5PCT, OVER64PCT, VULEOPCT)
  for (i in 4:10) {
    boyce <- ei%>%
      select(GIS_Tract, colnames(ei)[2],ACSTOTPOP,colnames(ei)[i])%>%
      mutate(NW_Left = .[[2]] * ACSTOTPOP * .[[4]],
             NW_Right = ACSTOTPOP * (1-.[[4]]),
             W_Left = .[[2]] * ACSTOTPOP * (1-.[[4]]),
             W_Right = ACSTOTPOP * .[[4]])%>%
      group_by(GIS_Tract)%>%
...
for (n in 32:34) {
  ei <- allbgs%>%
    select(GIS_County,colnames(allbgs)[n],ACSTOTPOP,MINORPCT, LOWINCPCT, LESSHSPCT, LINGISOPCT, UNDER5PCT, OVER64PCT, VULEOPCT)
  for (i in 4:10) {
    boyce <- ei%>%
      select(GIS_County, colnames(ei)[2],ACSTOTPOP,colnames(ei)[i])%>%
      mutate(NW_Left = .[[2]] * ACSTOTPOP * .[[4]],
             NW_Right = ACSTOTPOP * (1-.[[4]]),
             W_Left = .[[2]] * ACSTOTPOP * (1-.[[4]]),
             W_Right = ACSTOTPOP * .[[4]])%>%
...
for (n in 32:34) {
  ei <- allbgs%>%
    select(GIS_State,colnames(allbgs)[n],ACSTOTPOP,MINORPCT, LOWINCPCT, LESSHSPCT, LINGISOPCT, UNDER5PCT, OVER64PCT, VULEOPCT)
  for (i in 4:10) {
    boyce <- ei%>%
      select(GIS_State, colnames(ei)[2],ACSTOTPOP,colnames(ei)[i])%>%
      mutate(NW_Left = .[[2]] * ACSTOTPOP * .[[4]],
             NW_Right = ACSTOTPOP * (1-.[[4]]),
             W_Left = .[[2]] * ACSTOTPOP * (1-.[[4]]),
             W_Right = ACSTOTPOP * .[[4]])%>%
...
for (n in 32:34) {
  ei <- msaClean%>%
    select(GIS_MSA,colnames(msaClean)[n],ACSTOTPOP,MINORPCT, LOWINCPCT, LESSHSPCT, LINGISOPCT, UNDER5PCT, OVER64PCT, VULEOPCT)
  for (i in 4:10) {
    boyce <- ei%>%
      select(GIS_MSA, colnames(ei)[2],ACSTOTPOP,colnames(ei)[i])%>%
      mutate(NW_Left = .[[2]] * ACSTOTPOP * .[[4]],
             NW_Right = ACSTOTPOP * (1-.[[4]]),
             W_Left = .[[2]] * ACSTOTPOP * (1-.[[4]]),
             W_Right = ACSTOTPOP * .[[4]])%>%
